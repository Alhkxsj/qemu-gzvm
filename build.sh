...
commonCFlags="-O3 -fsanitize=undefined -fno-omit-frame-pointer -flto=thin -ffunction-sections -fdata-sections -mcpu=oryon-1 -fPIC -fno-semantic-interposition -ftls-model=global-dynamic"
libraryCFlags="$commonCFlags -DNDEBUG"
commonLdFlags="-fsanitize=undefined -flto=thin -Wl,--lto-O3 -Wl,-O3 -Wl,--gc-sections -Wl,--icf=all"
...
buildQemu() {
  ...
  "$mesonBin" compile -C "$qemuBuild" qemu-system-aarch64 qemu-gzvm -j"$nCpu"
}
packageQemu() {
  local qemuBinary="$qemuBuild/qemu-system-aarch64"
  local jniBinary="$qemuBuild/libqemu-gzvm.so"
  if [ ! -f "$qemuBinary" ] || [ ! -f "$jniBinary" ]; then
    echo "QEMU 构建产物不完整" >&2
    exit 1
  fi
  rm -rf "$qemuDir"
  ...
  "$strip" --strip-all "$jniBinary" -o "$qemuDir/libqemu-gzvm.so"

...
void virt_machine_done(Notifier *notifier, void *data)
{
    ...
    dtb_size = arm_load_dtb(info->dtb_start, info, info->dtb_limit, as, ms, cpu);
    if (dtb_size < 0) {
        exit(1);
    }
...
static void machvirt_init(MachineState *machine)
{
    ...
    if (vms->virt && !kvm_enabled() && !tcg_enabled()
       && !hvf_enabled() && !gzvm_enabled() && !qtest_enabled()) {
        error_report("mach-virt: %s does not support providing "
                     "Virtualization extensions to the guest CPU",
                     current_accel_name());
        exit(1);
...
static GPtrArray *virt_get_valid_cpu_types(const MachineState *ms)
{
    ...
    if (target_aarch64()) {
        ...
        if (kvm_enabled() || hvf_enabled() || whpx_enabled() || gzvm_enabled()) {
